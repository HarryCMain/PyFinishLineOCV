...
class RobotRaceTracker:
    def __init__(self):
        ...
        self.screen_width = 1920  # Adjust to your display resolution
        ...
        if self.screen_width/self.screen_height > self.cam_aspect:
            self.disp_height = self.screen_height
            self.disp_width = int(self.disp_height * self.cam_aspect)
        else:
            self.disp_width = self.screen_width
            self.disp_height = int(self.disp_width / self.cam_aspect)
        
        # Finish line parameters (relative to camera resolution)
        self.finish_line_y = int(self.cam_height * 0.5)
        self.finish_line_x1 = int(self.cam_width * 0.2)
        self.finish_line_x2 = int(self.cam_width * 0.8)
        ...
        cv2.setWindowProperty('Robot Race Tracker', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
        
    def scale_to_display(self, frame):
        """Scale camera frame to display size with letterboxing"""
        scaled = cv2.resize(frame, (self.disp_width, self.disp_height))
        display_frame = np.zeros((self.screen_height, self.screen_width, 3), dtype=np.uint8)
        
        # Center the scaled image
        x_offset = (self.screen_width - self.disp_width) // 2
        y_offset = (self.screen_height - self.disp_height) // 2
        display_frame[y_offset:y_offset+self.disp_height, x_offset:x_offset+self.disp_width] = scaled
        
        return display_frame, x_offset, y_offset
    ...
    def display_leaderboard(self, frame, x_offset, y_offset, scale):
        ...
        for i, (color, t) in enumerate(sorted_leaderboard):
            ...
            cv2.putText(frame, text, 
                       (base_x, base_y + (i+1)*line_height), 
                       cv2.FONT_HERSHEY_SIMPLEX, font_scale*0.85, (0, 255, 0), 2)
    
    def display_instructions(self, frame, x_offset, y_offset, scale):
        font_scale = 0.5 * scale
        line_height = int(30 * scale)
        base_x = x_offset + 10
        base_y = y_offset + self.disp_height - 30
    ...
    def run(self):
        while True:
            ...
            # Process frame in camera resolution
            ...
            display_frame, x_offset, y_offset = self.scale_to_display(processed_frame)
            scale_factor = self.disp_width / self.cam_width
            
            # Display mode text
            mode_text = "PAUSED" if self.paused else "RUNNING"
            cv2.putText(display_frame, mode_text, 
                       (self.screen_width - int(150 * scale_factor), int(30 * scale_factor)), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7 * scale_factor, (0, 0, 255), 2)
            
            # Display UI elements
            if not self.setup_mode:
                self.display_leaderboard(display_frame, x_offset, y_offset, scale_factor)
            self.display_instructions(display_frame, x_offset, y_offset, scale_factor)
